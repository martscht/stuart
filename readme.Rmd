...
The first row selects all the items our factor structure contains XXX0 The function lapply hereby returns a list with the same length as ords, but, just like we specified in the function itself, ordered. After that, using the do.call function, we now turn the newly made ordered ords-list to a data.frame, so we can use stuart for the item selection.

Next up we have to alter our objective. Since we are working with ordinal data, it's better to use the robust (rmsea.robust, cfi.robust) or the scaled (rmsea.scaled, cfi.scaled) fit statistics instead of the normal ones (rmsea, cfi) for modelling ordinal indicators with lavaan.
