...
The current stable version (0.10.2) is [available CRAN](https://cran.r-project.org/package=stuart) and can be installed using the usual approach:

```{r, eval = FALSE}
install.packages('stuart')
```

The current development version can be installed via the `install_github()`-function included in the `devtools` package
...
The current STUART version (0.10.2) provides four approaches to item selection, each in its own function:
