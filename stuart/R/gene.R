...
#' 
#' Per default, convergence is checked via the variance of the global-best values on the objective function, as proposed by Bhandari, Murthy, and Pal (2012). For generalizability over different functions provided to \code{objective}, variances are scaled to the first global-best found. To avoid false convergence in the early search, the lower of either 10\% of the generations or 10 generations must be completed, before convergence is checked. In this case the setting for \code{tolerance} pertains to the pure variance estimate and defaults to .0005 (or .005 when regarding the reinitialization process discussed below). Alternatively, the setting \code{'median'} checks for the raw difference between the objective function value of the generation-best and the median value of a generation. Here, the default is .05 (or .10 when regarding the reinitialization process). The setting \code{'geno.within'} checks for the variability of genotypes in a generation, by determining the relative frequency, with which each item is selected. Convergence is reached if this relative frequency is either \code{tolerance} (.9, by default - or .8 for the reinitialization process) or 1 - \code{tolerance} for all items within a generation. The setting \code{'geno.between'} checks convergence by tracking the changes between this variation in genotypes over subsequent generations. In this case the default for \code{tolerance} is .01 (or .05 for the reinitialization process).
#' 
